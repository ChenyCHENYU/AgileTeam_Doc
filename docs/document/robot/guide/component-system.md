...
此外，51 个业务组件已独立发布为 `@robot-admin/naive-ui-components` npm 包，通过 `RobotNaiveUiResolver` 实现自动导入，无需手动注册。
...
| **组件库组件** | `C_` | `C_Form`, `C_Table`, `C_Upload` | 51 个业务组件（自动导入） |
...
Robot Admin 的组件体系基于 `@robot-admin/naive-ui-components`（v0.8.1），包含 51 个业务组件：

<details>
<summary><b>📊 数据展示组件</b></summary>

- **`C_Table`** - 超级表格，支持虚拟滚动、树形结构、行拖抽、CSV/XLSX 导出、列配置持久化
...
- **`C_Form`** - 动态表单引擎，支持 8 种布局、脏检查、联动赋值、异步选项
...
<details>
<summary><b>🎨 媒体与编辑组件</b></summary>

- **`C_Editor`** - WangEditor 富文本编辑器
- **`C_Markdown`** - Markdown 编辑器
- **`C_Code`** - 代码编辑器
- **`C_FormulaEditor`** - 公式编辑器
- **`C_VideoPlayer`** - XGPlayer 视频播放器
- **`C_AudioPlayer`** - 音频播放器
- **`C_FilePreview`** - 文件预览（PDF/Excel/Word/图片）
- **`C_ImageCropper`** - 图片裁剪
- **`C_Signature`** - 电子签名
- **`C_QRCode`** / **`C_Barcode`** - 二维码与条形码

</details>

<details>
<summary><b>💬 交互业务组件</b></summary>

- **`C_Chat`** - 聊天组件，消息泡泡与会话列表
