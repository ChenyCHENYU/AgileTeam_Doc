...
例如企业用于宣传的官网页面、营销活动的推广落地页都非常适合使用预渲染技术，现代的构建工具都提供了预渲染的内置实现，例如这个教程： [用 Vite 更简单的解决 Vue3 项目的预渲染问题](https://github.com/cheny/vite-vue3-prerender-demo) ，就是通过 Vite 的内置功能来实现预渲染，最终也运用到了公司的业务上。
