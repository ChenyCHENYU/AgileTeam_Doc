...
如果已经看完 [工程化的前期准备](../get-familiar-quickly/guide.md) 一章，相信此时的已经对 Node 工程项目有了足够的认识了，在此之前的所有代码都是使用 JavaScript 编写的，接下来这一节，将开始介绍 TypeScript ，这是一门新的语言，但是上手非常简单。
...
<ClientOnly>
  <ImgWrap
    src="https://cheny-chenyu.oss-cn-chengdu.aliyuncs.com/my-agile-team-document/img/github-top-languages.jpg"
    dark="https://cheny-chenyu.oss-cn-chengdu.aliyuncs.com/my-agile-team-document/img/github-top-languages-dark.jpg"
...
要想知道自己为什么要用 TypeScript ，先从 JavaScript 不足方面说起，举一个非常小的例子：
...
:::tip
如果还没有体验这个 demo ，建议先按教程跑一下，然后跟着介绍来了解不同的 JavaScript 类型应该如何在 TypeScript 里定义，接下来的时间里，可以一边看，一边在 demo 里实践。
:::
...
有没有发现窍门？对！ TypeScript 对原始数据的类型定义真的是超级简单，跟 JavaScript 一样，就是转为全小写即可！
...
最常用的就是 `string[]` 这样的格式，只需要追加一个方括号 `[]` ，另外一种写法是基于 TS 的泛型 `Array<T>` ，两种方式定义出来的类型其实是一样的。
...
:::danger
如果一开始是 `[]` ，那就必须显式的指定数组类型（取决于 [tsconfig.json](#了解-tsconfig-json) 的配置，可能会引起报错）：
:::
...
而对于复杂的数组，比如数组里面的 item 都是对象，其实格式也是一样，只不过把原始数据类型换成 [对象的类型](#对象-接口) 即可，例如 `UserItem[]` 表示这是一个关于用户的数组对象列表。

### 对象（接口）

了解完数组，接下来看看对象的用法，对象也是引用类型，在 [数组](#数组) 的最后提到了一个 `UserItem[]` 的写法，这里的 `UserItem` 就是一个对象的类型定义。
...
为了降低上手门槛，统一使用 `interface` 来做介绍，它的写法与 Object 更为接近，事实上它也被用的更多。
